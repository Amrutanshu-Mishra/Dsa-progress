...
ll brute_force(ll n,vll &arr,ll d){
    ll count=0;
    for(int i=0;i<n-2;i++){
...
ll current_sol(ll n,ll d,vll &arr){
    ll n,d;
   cin>>n>>d;
   vll arr(n);
   loop(0,n){
...
//    cout<<arr1.size()<<endl;
   if(arr1.size()>2){
     for (ll i = 0; i < arr1.size()-2; i++)
    {
        // cout<<"YES"<<endl;
        for(ll j=i+1;j<arr1.size()-1;j++){
            // map<int,bool>visited;
            if(arr1[i]+arr1[j]<=d){
                int idx1=lower_bound(arr1.begin()+j+1,arr1.end(),d-arr1[i]-arr1[j])-arr1.begin();
                if(idx1<arr1.size()&&arr1[idx1]==d-arr1[i]-arr1[j]){
                    count+=m1[arr1[idx1]]*m1[arr1[i]]*m1[arr1[j]];
                }
            }
            if(arr1[i]+arr1[j]<=2*d){
                int idx1=lower_bound(arr1.begin()+j+1,arr1.end(),2*d-arr1[i]-arr1[j])-arr1.begin();
                if(idx1<arr1.size()&&arr1[idx1]==2*d-arr1[i]-arr1[j]){
                    count+=m1[arr1[idx1]]*m1[arr1[i]]*m1[arr1[j]];
                }
            }
        }
...
//    cout<<"YES"<<endl;
   loop(0,arr1.size()){
    if((arr1[i]*3)%d==0){
        count+=(m1[arr1[i]])*(m1[arr1[i]]-1)*(m1[arr1[i]]-2)/6;
    }
   }
   loop(0,arr1.size()){
    
        if(2*arr1[i]<=d){
            int idx1=lower_bound(arr1.begin()+i+1,arr1.end(),d-2*arr1[i])-arr1.begin();
            if(idx1<arr1.size()&&arr1[idx1]==d-2*arr1[i]){
                count+=m1[arr1[idx1]]*m1[arr1[i]]*(m1[arr1[i]]-1)/2;
            }
        }
        if(2*arr1[i]<=2*d){
            int idx1=lower_bound(arr1.begin()+i+1,arr1.end(),2*d-2*arr1[i])-arr1.begin();
            if(idx1<arr1.size()&&arr1[idx1]==2*d-2*arr1[i]){
                count+=m1[arr1[idx1]]*m1[arr1[i]]*(m1[arr1[i]]-1)/2;
            }
        }
...
void solve(){
   ...
   ll ans1=current_sol(n,d,arr);
   ll ans2=brute_force(n,arr,d);
   cout<<ans1<<ans2<<endl;
}  
int main(){ 
    int t;
