...
bool driver(ll d,vll &arr,ll n,ll p,ll k){
    ll idx1=lower_bound(arr.begin(),arr.end(),p-d)-arr.begin();
    ll idx2=upper_bound(arr.begin(),arr.end(),p+d)-arr.begin()-1;
    if(idx2-idx1+1>=k){
        return true;
    }
    return false;
...
void solve(){
   ...
   while (q--)
   {
        ...
        cin>>p;
        ll low=1;
        ll high=arr[n-1]-arr[0]+1;
        ll ans=high;
        ll mid;
        while (low<=high)
        {
            mid=(low+high)/2;
            if(driver(mid,arr,n,p,k)){
                high=mid-1;
                ans=min<ll>(ans,mid);
            }
            else{
